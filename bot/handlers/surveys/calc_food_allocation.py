...
async def calc_food_allocation_button_handler(message: Message, survey: SurveyContext) -> None:
    ...
    await survey.state.set_state(CalcFoodAllocationSurvey.first_dry_mass)
...
async def calc_food_allocation_survey_cancel_button_handler(message: Message, survey: SurveyContext) -> None:
...
async def calc_food_allocation_survey_first_dry_mass_handler(message: Message, survey: SurveyContext) -> None:
    await survey.add_messages_to_delete(message.message_id)

    first_dry_mass = parse_float(message.text)
    await survey.state.update_data(first_dry_mass=first_dry_mass)
    await survey.state.set_state(CalcFoodAllocationSurvey.second_dry_mass)
...
async def calc_food_allocation_survey_undo_survey_handler(message: Message, survey: SurveyContext) -> None:
...
async def calc_food_allocation_survey_second_dry_mass_handler(message: Message, survey: SurveyContext) -> None:
    await survey.add_messages_to_delete(message.message_id)

    second_dry_mass = parse_float(message.text)
    await survey.state.update_data(second_dry_mass=second_dry_mass)
    await survey.state.set_state(CalcFoodAllocationSurvey.total_ready_mass)
...
async def calc_food_allocation_survey_undo_first_dry_mass_handler(message: Message, survey: SurveyContext) -> None:
...
async def calc_food_allocation_survey_total_ready_mass_handler(message: Message, survey: SurveyContext) -> None:
    await survey.add_messages_to_delete(message.message_id)
    await survey.clear_messages(bot=message.bot, chat_id=message.chat.id, subset=slice(1, None))

    total_ready_mass = parse_float(message.text)
    await survey.state.update_data(total_ready_mass=total_ready_mass)
...
async def calc_food_allocation_survey_undo_second_dry_mass_handler(message: Message, survey: SurveyContext) -> None:
    await survey.clear_messages(
        bot=message.bot,
        chat_id=message.chat.id,
        group_name=CalcFoodAllocationSurvey.second_dry_mass.state,
...
async def calc_food_allocation_survey_invalid_mass_handler(message: Message, survey: SurveyContext) -> None:
